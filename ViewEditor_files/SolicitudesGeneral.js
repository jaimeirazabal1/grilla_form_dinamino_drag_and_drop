...
function ClearFormControls() {
    $('#CreateSolicitudes').trigger('reset');
    $('#CreateSolicitudes').find(':input').each(function () {
        switch (this.type) {
            case 'password':
            case 'number':
...
$(document).ready(function () {
    $("form#CreateSolicitudes").submit(function (e) {
        e.preventDefault();
        return false;
    });
    ...
    $("#GuardarYCopia").click(function () {
        if (CheckValidation())
            SendSolicitudesData(function (currentId) {
                $("#FolioId").val("0");
...
function DisplayBusinessRule() {
    if (!isdisplayBusinessPropery) {
        $('#CreateSolicitudes').find('.col-sm-8').each(function () {
